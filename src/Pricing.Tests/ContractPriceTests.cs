...
namespace Pricing.Tests
{
    ...
    public class ContractPriceTests
    {
        ...
        {
            _priceBuilder.AddContractPrice("33333", 15.99m, "AA11AA");
            var engine = new Engine(_priceBuilder.Rules);
            _itemBuilder.AddItem("33333", 3.99m);
            _customer.Setup(c => c.Reference).Returns("AA11AA");

            var actual = engine.CalculatePrices(_itemBuilder.Items, _customer.Object);
